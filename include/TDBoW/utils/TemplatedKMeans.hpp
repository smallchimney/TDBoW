...
protected:
    ...
    typedef TemplatedCheckPoint<Scalar, DescriptorUtil::DescL> CheckPointUtil;
...
public:
    typedef std::function<void(CheckPointUtilPtr&, const size_t&,
            const std::vector<DescriptorConstPtr>&, DescriptorArray&,
            DistanceCallback, MeanCallback)> InitMethods;
     ...
     * @param  _CheckPoint      Checkpoint handler, kept {@code nullptr}
     *                          if no need to log.
     * @param  _Init            Initial seed selected methods, default
     *                          by K-Means++.
     * @param  _DistF           Distance function.
     * @param  _MeanF           Mean value function.
     * @return                  The cluster result of each descriptor
     */
    std::vector<size_t> process(const std::vector<DescriptorConstPtr>& _Descriptors,
                 DescriptorArray& _Centers,
                 std::vector<std::vector<DescriptorConstPtr>>& _Clusters,
                 CheckPointUtilPtr& _CheckPoint = nullptr,
     ...
     * @author smallchimney
     * @param  _CheckPoint    Checkpoint handler pointer (maybe nullptr).
     * @param  _K             Excepted centers number.
     * @param  _Descriptors   Input descriptors.
     * @param  _Centers (out) resulting clusters.
     * @param  _DistF         Distance function.
     */
    static void initiateClustersKM(CheckPointUtilPtr& _CheckPoint,
            const size_t& _K, const std::vector<DescriptorConstPtr>& _Descriptors,
     ...
     * @param  _DistF         Distance function.
     */
    static void initiateClustersKMpp(CheckPointUtilPtr& _CheckPoint,
            const size_t& _K, const std::vector<DescriptorConstPtr>& _Descriptors,
            DescriptorArray& _Centers, DistanceCallback _F, MeanCallback _M);
     ...
     * @param  _DistF         Distance function.
     */
    static void initiateClustersKM2nd(CheckPointUtilPtr& _CheckPoint,
            const size_t& _K, const std::vector<DescriptorConstPtr>& _Descriptors,
            DescriptorArray& _Centers, DistanceCallback _F, MeanCallback _M);
...
template <class DescriptorUtil>
std::vector<size_t> TemplatedKMeans<DescriptorUtil>::process(
        ...
        CheckPointUtilPtr& _CheckPoint, InitMethods _Init,
    ...
    if(_Descriptors.size() <= m_ulK) {
        ...
        std::vector<size_t> belong(_Descriptors.size());
#ifdef FOUND_OPENMP
    ...
    // maybe out of range, but is safe because space is reserved
    ...
    if(_CheckPoint && _CheckPoint -> completed(CheckPointUtil::CLUSTER_ITER)) {
        firstTime = false;
        _CheckPoint -> ckptLoadIter(
                iterCount, prevLoss, _Descriptors.size(), previousBelong);
        _CheckPoint -> ckptLoaded();
        _Centers.resize(m_ulK);
        _Clusters.assign(_Centers.size(), std::vector<DescriptorConstPtr>());
        for(size_t i = 0; i < _Descriptors.size(); i++) {
            _Clusters[previousBelong[i]].emplace_back(_Descriptors[i]);
        }
    }
    ...
    while(true) {
        ...
        if(firstTime) {
            // random sample
            if(_CheckPoint) {
                bool inInit = _CheckPoint -> getStatus()
                        == CheckPointUtil::CLUSTER_INIT;
                if(!inInit) {
                    _CheckPoint -> setStatus(CheckPointUtil::CLUSTER_INIT);
                }
                _Init(_CheckPoint, m_ulK, _Descriptors, _Centers, _DistF, _MeanF);
                if(!inInit) {
                    _CheckPoint -> setStatus(CheckPointUtil::CLUSTER_ITER);
                }
            } else {
                _Init(_CheckPoint, m_ulK, _Descriptors, _Centers, _DistF, _MeanF);
            }
            ...
            if(firstTime) {
                const auto line = TDBOW_LOG(
                        "Bad cluster founded, re-run the k-means iterations.");
                if(_CheckPoint && _CheckPoint -> valid()) {
                    _CheckPoint -> output(line, CheckPointUtil::ERROR);
                } else {
                    std::cerr << line;
                }
                continue;
        ...
        if(stride && !std::is_same<Scalar, typename DescriptorUtil::binary_type>()) {
            ...
            kdTree.buildIndex();
            if(_CheckPoint && _CheckPoint -> console()) {
                // In case logging required
                BarProgress progress(_Descriptors.size());
#ifdef FOUND_OPENMP
        ...
        if(iterCount++ && loss > prevLoss) {
            ...
            return previousBelong;
        }

        if(_CheckPoint && _CheckPoint -> valid()) {
            _CheckPoint -> str() << "<Iter " << iterCount << ": loss "
                    << loss / _Descriptors.size() << '>' << std::endl;
            _CheckPoint -> output();
            // No checkpoint during cluster init in this version
            if(_CheckPoint -> completed(CheckPointUtil::CLUSTER_ITER)) {
                _CheckPoint -> ckptSaveIter(iterCount, loss, currentBelong);
                _CheckPoint -> ckptDone();
            }
        }
...
template <class DescriptorUtil>
void TemplatedKMeans<DescriptorUtil>::initiateClustersKM(
        CheckPointUtilPtr& _CheckPoint, const size_t& _K,
        const std::vector<DescriptorConstPtr>& _Descriptors,
        DescriptorArray& _Centers, DistanceCallback _F, MeanCallback _M) {
    if(_CheckPoint) {
        _CheckPoint -> output("Start k-means initialization.\n");
    }
    // Random K seeds
    _Centers.clear();
    ...
    if(_CheckPoint) {
        _CheckPoint -> output("Finish k-means initialization.\n");
    }
}
...
template <class DescriptorUtil>
void TemplatedKMeans<DescriptorUtil>::initiateClustersKM2nd(
        CheckPointUtilPtr& _CheckPoint, const size_t& _K,
        const std::vector<DescriptorConstPtr>& _Descriptors,
        DescriptorArray& _Centers, DistanceCallback _F, MeanCallback _M) {
    ...
    if(_Descriptors.size() <= LIMIT) {
        initiateClustersKMpp(_CheckPoint, _K, _Descriptors, _Centers, _F, _M);
        return;
    }
    ...
    if(_CheckPoint) {
        _CheckPoint -> str() << "Start k-means 2nd initialization at "
                << seeds.size() << '.' << std::endl;
        _CheckPoint -> output();
    }
    std::vector<std::vector<DescriptorConstPtr>> ignored;
    TemplatedKMeans<DescriptorUtil>(_K).process(seeds,
            _Centers, ignored, _CheckPoint, initiateClustersKMpp, _F, _M);
    if(_CheckPoint) {
        _CheckPoint -> str() << "Finish k-means 2nd initialization at "
                << seeds.size() << '.' << std::endl;
        _CheckPoint -> output();
    }
}
